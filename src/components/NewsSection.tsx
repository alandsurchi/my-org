...
const NewsSection = () => {
  ...
  const getTabIcon = (tabValue: string) => {
    ...
    return icons[tabValue as keyof typeof icons] || Building;
  };

  const renderNewsGrid = (newsItems: any[]) => (
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
      {filterNews(newsItems).map((item, index) => {
        const IconComponent = getTabIcon(item.category);
        const title = getLocalizedText(item, 'title');
        const description = getLocalizedText(item, 'description');
        ...
        return (
          <Card key={item.id} className="group bg-white/80 backdrop-blur-sm border-0 shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden rounded-3xl hover-lift fade-in-on-scroll" style={{ animationDelay: `${index * 0.1}s` }}>
            <div className="relative overflow-hidden">
              <img 
                src={item.image_url || 'https://images.unsplash.com/photo-1497486751825-1233686d5d80?w=400&h=250&fit=crop'} 
                alt={title}
  ...
  if (loading) {
    return (
      <section id="news" className="py-24 bg-gradient-to-br from-purple-50/30 via-blue-50/20 to-gray-50">
  ...
  if (error) {
    return (
      <section id="news" className="py-24 bg-gradient-to-br from-purple-50/30 via-blue-50/20 to-gray-50">
        <div className="container mx-auto px-4">
          <div className="text-center text-red-600">Error loading news: {error}</div>
        </div>
      </section>
    );
  }
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <Tabs defaultValue="placesVisited" className="w-full">
          <TabsList className="grid w-full grid-cols-2 lg:grid-cols-4 mb-12 bg-white/80 backdrop-blur-sm shadow-lg rounded-2xl p-2 h-auto">
            {['placesVisited', 'visitors', 'certificatesReceived', 'certificatesAwarded'].map((key) => {
          ...
          <TabsContent value="placesVisited" className="mt-8">
            {renderNewsGrid(getNewsByCategory('placesVisited'))}
          </TabsContent>

          <TabsContent value="visitors" className="mt-8">
            {renderNewsGrid(getNewsByCategory('visitors'))}
          </TabsContent>

          <TabsContent value="certificatesReceived" className="mt-8">
            {renderNewsGrid(getNewsByCategory('certificatesReceived'))}
          </TabsContent>

          <TabsContent value="certificatesAwarded" className="mt-8">
            {renderNewsGrid(getNewsByCategory('certificatesAwarded'))}
          </TabsContent>
        </Tabs>
